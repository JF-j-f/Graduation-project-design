<%@ page contentType="text/html;charset=UTF-8" language="java" %>
    <%@ page import="com.music.javabean.*, com.music.dao.*, java.util.*" %>
        <% // 检查用户是否登录 User user=(User) session.getAttribute("user"); if (user==null) {
            response.sendRedirect("index.jsp"); return; } // 获取歌单信息（由 PlaylistServlet 传递） Playlist playlist=(Playlist)
            request.getAttribute("playlist"); if (playlist==null) { response.sendRedirect("user.jsp"); return; } //
            获取歌单中的歌曲列表 List<Song> songs = playlist.getSongs();
            if (songs == null) {
            songs = new ArrayList<>();
                }
                    ...
                    <div class="container" style="padding-top: 100px; max-width: 1200px; margin: 0 auto;">
                        ...
                        <section class="playlist-songs" style="margin-top: 3rem;">
                            ...
                            <% if (songs.isEmpty()) { %>
                                ...
                                <% } else { %>
                                    <div class="song-list">
                                        <% for (int i=0; i < songs.size(); i++) { Song song=songs.get(i); boolean
                                            isFavorited=favoriteDAO.isFavorite(user.getId(), song.getId()); %>
                                            <div class="song-item fade-in" id="song-item-<%= song.getId() %>">
                                                ...
                                                <div class="song-cover">
                                                    <% if (song.getCoverImage() !=null &&
                                                        ...
                                                        coverUrl=song.getCoverImage(); if (!coverUrl.startsWith("http")
                                                        && !coverUrl.startsWith("/")) {
                                                    ...
                                                    <button class="favorite-btn-ajax <%= isFavorited ? " favorited" : ""
                                                        ...
                                                        data-action="<%= isFavorited ? "remove" : "add" %>"
                                                            data-song-id="<%= song.getId() %>"
                                                                ...
                                                                1.25rem; cursor: pointer; padding: 0.5rem; color: <%=
                                                                    isFavorited ? "red" : "#666" %>;">
                                                                    <%= isFavorited ? "❤️" : "🤍" %>
                                                    </button>
